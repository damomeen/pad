...
#include <inttypes.h>
#include <rofl/common/openflow/experimental/matches/ictp_matches.h>
#include <rofl/common/openflow/experimental/actions/ictp_actions.h>
...
ethswitch::handle_dpath_open(
		cofdpt *dpt)
{
    std::cerr << "datapath connected" << std::endl; 
    ...
    fe.instructions.back().actions.next() = cofaction_push_ictp(dpt->get_version(), 0x9100);
    fe.instructions.back().actions.next() = cofaction_set_field(dpt->get_version(), rofl::coxmatch_ofx_ictp_nid(0x11));
    fe.instructions.back().actions.next() = cofaction_set_field(dpt->get_version(), rofl::coxmatch_ofx_ictp_csn(0x2233));
    fe.instructions.back().actions.next() = cofaction_output(dpt->get_version(), 2);
    
    std::cerr << cofaction_pop_ictp(dpt->get_version(), 0x800).c_str() << std::endl;  
